...
<template>
  ...
  <div
    ...
    :style="`--background-image: url(${
      settings.background ? settings.background : '/background.png'
    });`"
...
onMounted(() =>
  App.setThemeLayout(
    (<ComponentPublicInstance>proxy).$el.offsetParent.classList,
    settings.value.theme.layout
  )
);
  ...
  setThemeLayout: () =>
    App.setThemeLayout(
      ...
      settings.value.theme
        .layout /* === 'light' ? settings.value.theme.primary.toLowerCase().replace(' ', '') : settings.value.theme.layout */
    ),
});
</script>
