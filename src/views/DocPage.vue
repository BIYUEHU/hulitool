...
<script setup lang="ts">
    ...
    const progress = ref(false), dataStat = ref({total: 0, day: 0}),
        dataRes = ref(null), /* dataOther = ref(null), */
        Component = ref(null), dataDoc = ref(null);
    ...
    /* 请求具体的API获取数据 */
    const getData = (dataReq = null, auto = true) => {
        if (!http[dataDoc.value.http] || !auto) return;

        progressView();
        const Promise = http[dataDoc.value.http](dataReq);
        /* 判断是多个请求还是单个 */
        if (Array.isArray(Promise)) {
            const Res = [];
            for (let init = 0; init < Promise.length; init++) {
                // if (Promise[init] !== null && Promise[init].then) {
                Promise[init].then(res => {
                    Res[init] = res.data;
