<template>
    <div class="mdui-dialog" id="dialog-settings">
            ...
            <div>
                ...
                <form class="mdui-row-xs-1 mdui-row-sm-2 mdui-row-md-3">
                    <div v-for="color in ThemeData.primary" :key="color"
                        ...
                        <label class="mdui-radio mdui-m-b-1">
                            <input v-model="settings.theme.primary" type="radio" name="theme-primary" :value="handle(color)">
                            <i class="mdui-radio-icon"></i>
                            {{ color }}
                        </label>
                    </div>
        ...
        <div class="mdui-dialog-actions">
            <button class="mdui-btn mdui-ripple mdui-float-left"
                @click="settings.setDefaultTheme()">恢复默认设置</button>
            <button class="mdui-btn mdui-ripple" mdui-dialog-confirm="" @click="proxy.$mdui.snackbar('保存成功')">确定</button>
        </div>
    </div>
</template>
