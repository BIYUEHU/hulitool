<template>
  <div>
    <div class="bg">
      ...
      <div class="container">
        <div class="text" :class="{ 'text-move': isAnimation.text }">
          功德+1
        </div>
        <img
          id="muyu"
          class="muyu"
          :class="{ 'muyu-click': isAnimation.muyu }"
          @click="click"
    ...
    <div>
      <input
        ...
        v-model="autoTime"
        type="text"
        placeholder="自动点击间隔时间(0关闭) 单位:秒"
      />
      <button
        @click="setAutoTime"
        class="mdui-btn mdui-btn-dense mdui-color-theme-accent mdui-ripple"
      >
        <i class="mdui-icon material-icons">timer</i>设置自动时间
      </button>
      <button
        ...
        class="mdui-btn mdui-btn-dense mdui-color-theme-accent mdui-ripple"
      >
        <i class="mdui-icon material-icons">clear</i>清除功德
      </button>
    </div>
  </div>
</template>
...
const setAutoTime = () => {
  ...
  if (autoTime.value)
    timerId = Number(setInterval(() => click(), autoTime.value * 1000));
};
setAutoTime();
</script>
