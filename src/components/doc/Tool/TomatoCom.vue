    ...
    <div style="margin-top: 20px;">
        <card class="mdui-card mdui-hoverable">
            <div class="mdui-card-header-title">
                <h4 class="mdui-text-color-theme-accent"><strong>番茄工作法</strong></h4>
            </div>
            <div class="mdui-card-content container">
                <div class="mdui-textfield">
                    <label class="mdui-textfield-label">番茄时长（分钟）</label>
                    <input class="mdui-textfield-input" v-model="tomatoDuration" maxlength="6" type="number" />
                </div>
                <div class="mdui-textfield">
                    <label class="mdui-textfield-label">休息时长(分钟)</label>
                    <input class="mdui-textfield-input" v-model="restDuration" maxlength="6" type="number" />
                </div>
            ...
            <div>
                <button @click="startTomato" v-if="!state"
                    class="child mdui-btn mdui-btn-dense mdui-color-theme-accent mdui-btn-raised mdui-ripple"><i class="mdui-icon material-icons">access_alarm</i>开始专注</button>
                <button @click="stopTomato()" v-else
                    class="child mdui-btn mdui-btn-dense mdui-btn-raised mdui-ripple"><i class="mdui-icon material-icons">stop</i>停止番茄</button>
            </div>
        </card>
        ...
        <card class="mdui-card mdui-hoverable mdui-textfield">
            ...
            <div class="mdui-card-content container">
                ...
                <h1 class="time" v-if="state" :style="{ color: state === 1 ? 'orange' : 'lightgreen' }">
        ...
        <card class="mdui-card mdui-hoverable mdui-textfield">
            <div class="mdui-card-header-title">
                <h4 class="mdui-text-color-theme-accent"><strong>什么是番茄？</strong></h4>
            </div>
            <div class="mdui-card-content container">
                <i>番茄工作法是简单易行的时间管理方法。</i>
                <i>使用番茄工作法，选择一个待完成的任务，将番茄时间设为25分钟，专注工作，中途不允许做任何与该任务无关的事，直到番茄时钟响起，然后进行短暂休息一下（5分钟就行），然后再开始下一个番茄。每4个番茄时段多休息一会儿。</i>
                <i> 番茄工作法极大地提高了工作的效率，还会有意想不到的成就感。</i>

                <h5>番茄工作法流程</h5>
                <li>每天开始的时候规划这一天要完成的几项任务，将任务逐项写在列表里（或记在软件的清单里）</li>
...
const stopTomato = (complete: boolean = false) => {
    ...
    if (autoTomato.value && complete) {
        if (state.value === STATE.REST) { startTomato(); return; }
