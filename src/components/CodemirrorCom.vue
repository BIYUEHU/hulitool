...
<template>
    <codemirror v-model="code" :placeholder="options.tips ?? 'Code gose here...'" :style="options.style ?? { height: '400px' }" :autofocus="true"
        :indent-with-tab="true" :tabSize="options.tabsize ?? 4" :extensions="extensions" />
</template>
...
for (const path in modules) {
    modules[path]().then((mod: unknown) => {
        if (path.match(`/@codemirror/lang-${options.lang}/dist/index.js`)) {
            console.log((<any>mod)[options.lang]());
            extensions[0] = [(<any>mod)[options.lang]()]
        }
    })
}
